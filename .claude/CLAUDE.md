...
ever added. Direct and group conversations are their own MLS groups
(a direct one keyed on the sorted pair, hashed); their name and
participants travel inside the group as a `meta` message, sent again
whenever devices are added. Supervision is key membership: the settings'
`superviseMessagesUpTo` tier puts every parent's devices in a thread with a
child at or below it (`ConversationAudience`), any device in the thread
reconciles it on sync, and a `readers` message announces each change. The
